...
func SaveAndExec(codesContent string, requirementTxtContent string) string {
	...
	if !status {
		 utils.LogInstance.WithFields(logrus.Fields{
			 ...
			 "folderId": folderId,
		 }).Info("go-run is trying to execute AWS CDK but failed.")
	...
	}).Info("go-run is trying to save AWS lambda codes to the path.")
	...
	if len(requirementTxtContent) >= 0 {
		...
		}).Info("go-run is trying to save requirement.txt to the path.")
...
func InitAwsCdkTask(folderId string) bool{
	...
	utils.LogInstance.WithFields(logrus.Fields{
		"path": cdkBaseFolder,
	}).Info("go-run is creating directory.")
	err := os.MkdirAll(cdkBaseFolder, os.ModePerm)
	if err != nil {
		utils.LogInstance.WithFields(logrus.Fields{
			"path": cdkBaseFolder,
			"error": err,
		}).Error("go-run failed to create directories.")
                return false
        }
	...
	if err != nil {
		utils.LogInstance.WithFields(logrus.Fields{
			"error": err,
		}).Error("go-run failed to get current working dir.")
		return false
	}
	...
	if err != nil {
		utils.LogInstance.WithFields(logrus.Fields{
			...
			"error": err,
		}).Error("go-run failed to execute cdk script.")
	...
	utils.LogInstance.WithFields(logrus.Fields{
		"cdkScriptPath": cdkScriptPath,
		"stdout": output,
	}).Info("go-run executed cdk script successfully.")
        return true
}
